"""
model.py — ParameterizedExperienceModel

整合四个组件：
  Encoder（冻结）→ MemoryBank（显式 CRUD）→ ExperienceAggregator（唯一可训练）
  → 冻结 Qwen2.5-Math-7B-Instruct
...
class ParameterizedExperienceModel(nn.Module):
    ...
    def __init__(self, config: Config) -> None:
        ...
        # ── 2. 经验向量库 M ────────────────────────────────────────────────
        self.memory = MemoryBank(N=config.N, d=config.d_enc, device=self.device)

        # ── 3. Experience Aggregator（可训练） ─────────────────────────────
        ...
        # ── 4. LLM（冻结） ────────────────────────────────────────────────
        print(f"[Model] 加载 LLM: {config.llm_name}  (dtype={self.dtype})")
    ...
    def generate_rollouts(
        ...
        prefix_mask = torch.ones(B, self.config.L, device=self.device, dtype=q_mask.dtype)
    ...
    def compute_logprob(
        ...
        """
        q_enc = self.tokenizer(question_text, return_tensors="pt").to(self.device)
        o_enc = self.tokenizer(output_text, return_tensors="pt").to(self.device)
